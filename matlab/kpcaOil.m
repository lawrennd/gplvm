% KPCAOIL Demonstrate kernel parameter selection with the sub-set of the oil data.

randn('seed', 1e5)
rand('seed', 1e5)
load ..\3Class

latentDim = 2;

% Select data
numData = 100;
indices = randperm(1000);
indices = indices(1:numData);
Y = DataTrn(indices, :);
lbls = DataTrnLbls(indices, :);


meanData = mean(Y);
Y = Y - repmat(meanData, numData, 1);
beta = 1e6;
dataDim = size(Y, 2);
gamma = linspace(0.1, 2, 100);
for i = 1:length(gamma)
  theta = [gamma(i) 0.1 beta];
  Ky = computeKernel(Y, theta);
  [U, D] = eig(Ky);
  [Lambda, index] = sort(-diag(D));
  Lambda = -Lambda;
  U = U(:, index);
  
  X = zeros(numData, latentDim);
  L(1, 1) = sqrt(Lambda(1) -1/beta);
  L(2, 2) = sqrt(Lambda(2) -1/beta);
  X(:, 1) = U(:, 1);
  X(:, 2) = U(:, 2);
  X = X*L;
  Kx = X*X'+1/beta*eye(numData); 
  invKx = pdinv(Kx);
  
  KyinvKx = Ky*invKx;
  val(i) = -dataDim*numData/2 - dataDim/2*logdet(KyinvKx) + dataDim/2*trace(KyinvKx);
  returnVal = [];
  
  symbol{1} = 'r+';
  symbol{2} = 'bo';
  symbol{3} = 'mx';
%  figure, hold on
%  for j = 1:size(X, 1)
%    labelNo = find(lbls(j, :));
%    plot(X(j, 1), X(j, 2), symbol{labelNo})
%  end
%  title([num2str(gamma(i)) ': ' num2str(val(i))]);
end
plot(gamma, val)