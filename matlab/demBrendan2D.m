...
model = gplvmFit(X, Y, theta, numActive, optionsKernel, ...
		      options, extIters)
  
% compute the kernel from results
K = model.Kstore(model.I, :);
invK = model.Linv'*model.Linv;
theta = exp(model.kern.lntheta);

K = model.kern.Kstore(model.I, :);
invK = pdinv(K);

% Visualise the results
gplvmVisualise(model.X, model.y, invK, theta, [], meanData, model.I, 'imageVisualise', ...
	       'imageModify', [20 28]);
